...
1. Use the release asset `blender_goh_gem_exporter-1.4.1.zip`, or create a zip that contains the `blender_goh_gem_exporter` folder at the root.
2. Open Blender.
...
- `Scale Factor = 20`
- `Flip V = On`
- `Defer Basis Flip = On` for GOH-native model inspection that matches SOEdit/game display while preserving the original mirrored `basis` for export
- `Defer Basis Flip = Off` only when you intentionally need to inspect the raw mirrored file-space parent
